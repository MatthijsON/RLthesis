...
def jit_run(state, n_times):
    ...
    function(state)
    start = timeit.default_timer()
    measure3 = pyRAPL.Measurement('test3')
    measure3.begin()
    for i in range(n_times):
    ...
    measure3.end()
    energy = jnp.array(measure3.result.pkg)
    return time_req2, energy
...
# measure2 = pyRAPL.Measurement('test2')
# measure2.begin()
for i in range(n_times_test):
    for j in n_times_run:
        ...
        run_jit_run, energy = jit_run(state,j)
        times_jit_run.append(run_jit_run)
        energy_amounts.append(jnp.sum(energy))
# measure2.end()
# results = jnp.array(measure2.result.pkg) / 1e6
...
total_energy = jnp.sum(jnp.array(energy_amounts)) / 1e6
print(total_energy)
print(total_jit)

import numpy as np
values_test = np.loadtxt('jax_time_data.csv')
total_values = jnp.sum(values_test)
print(total_values)
# name1 = 'jax_time_data10'
# name2 = 'jax_energy_data10'
#
# filename = f'{name1}.csv'
# filename2 = f'{name2}.csv'
#
...
#     writer.writerows([[value] for value in times_jit_run])
#
# with open(filename2, 'w', newline='') as file:
#     writer = csv.writer(file)
#
#     writer.writerows([[value] for value in energy_amounts])
