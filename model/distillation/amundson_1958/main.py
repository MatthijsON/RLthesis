...
class Model:
    def __init__(self, components: list = None, F: float = 0., P: float = 101325.,
        ...
        self.L = jnp.zeros(self.num_stages)
        self.V = jnp.zeros(self.num_stages)
        self.L_old = jnp.zeros(self.num_stages)
        self.V_old = jnp.zeros(self.num_stages)
        self.F = jnp.zeros(self.num_stages)
        #self.F[self.feed_stage] = self.F_feed
        self.F = self.F.at[self.feed_stage].set(self.F_feed)
        self.z = {
            key: jnp.zeros(self.num_stages) for key in components
        }
        self.l = {
            key: jnp.zeros(self.num_stages) for key in components
        }
        for component in self.components:
            ...
            self.z[component] = self.z[component].at[feed_stage].set(self.z_feed[component])
        self.T_feed = self.T_feed_guess
        self.T = jnp.zeros(self.num_stages)
        self.T_old = jnp.zeros(self.num_stages)
        self.K = {key: jnp.zeros(self.num_stages) for key in self.components}
    ...
    def update_K_values(self):
        ...
        for c in self.components:
            ...
            self.K[c] = self.K[c].at[:].set(self.K_func[c].eval_SI(self.T[:], self.P_feed))
        #self.z[component] = self.z[component].at[feed_stage].set(self.z_feed[component])
        #self.T_old[:] = self.T[:]
        self.T_old = self.T_old.at[:].set(self.T[:])
    ...
    def update_T_values(self):
        ...
        for i in range(self.num_stages):
            ...
            self.T = self.T.at[i].set(self.T_old[i] + self.df * (
    ...
    def initialize_flow_rates(self):
        ...
        self.L = self.L.at[:self.feed_stage].set(self.RR * self.D)
        #self.L[self.feed_stage:self.N] = self.RR * self.D + self.F_feed
        self.L = self.L.at[self.feed_stage:self.N].set(self.RR * self.D + self.F_feed)
        #self.L[self.N] = self.B
        self.L = self.L.at[self.N].set(self.B)
        #self.V[1:] = self.RR * self.D + self.D
        self.V = self.V.at[1:].set(self.RR * self.D + self.D)
    ...
    def solve_component_mass_bal(self, component):
        ...
        self.l[component] = self.l[component].at[:].set(solve_diagonal(A, B, C, D))

    def update_flow_rates(self):
        for i in self.stages:
            #self.L[i] = sum(self.l[c][i] for c in self.components)
            self.L = self.L.at[i].set(sum(self.l[c][i] for c in self.components))

        #self.V[0] = 0.  # total condenser
        self.V = self.V.at[0].set(0)
        #self.V[1] = (self.RR + 1.) * self.D
        self.V = self.V.at[1].set((self.RR + 1.) * self.D)
        for i in range(2, self.num_stages):
            #self.V[i] = self.L[i - 1] + self.D - sum(self.F[k] for k in range(i))
            self.V = self.V.at[i].set(self.L[i - 1] + self.D - sum(self.F[k] for k in range(i)))

    def solve_energy_balances(self):
        """Solve energy balances"""

        #self.L_old[:] = self.L[:]
        self.L_old = self.L_old.at[:].set(self.L[:])
        #self.V_old[:] = self.V[:]
        self.V_old = self.V_old.at[:].set(self.V[:])

        BE = jnp.zeros(self.num_stages)
        CE = jnp.zeros(self.num_stages)
        DE = jnp.zeros(self.num_stages)

        # total condenser
        #BE[0] = 0.
        BE = BE.at[0].set(0)
        #CE[0] = self.h_j_rule(0) - self.H_j_rule(1)
        CE = CE.at[0].set(self.h_j_rule(0) - self.H_j_rule(1))
        #DE[0] = self.F[0] * self.h_feed_rule(0) + self.Q_condenser_rule()
        DE = DE.at[0].set(self.F[0] * self.h_feed_rule(0) + self.Q_condenser_rule())

        # stages 1 to N-1
        for j in range(1, self.N):
            #BE[j] = self.H_j_rule(j) - self.h_j_rule(j - 1)
            BE = BE.at[j].set(self.H_j_rule(j) - self.h_j_rule(j - 1))
            #CE[j] = self.h_j_rule(j) - self.H_j_rule(j + 1)
            CE = CE.at[j].set(self.h_j_rule(j) - self.H_j_rule(j + 1))
            ...
            DE = DE.at[j].set(self.F[j] * self.h_feed_rule(j) - self.D * (self.h_j_rule(j - 1) - self.h_j_rule(j)) \
                    - sum(self.F[k] for k in range(j + 1)) * self.h_j_rule(j) \
                    + sum(self.F[k] for k in range(j)) * self.h_j_rule(j - 1))

        # partial reboiler
        # BE[self.N] = self.H_j_rule(self.N) - self.h_j_rule(self.N - 1)
        BE = BE.at[self.N].set(self.H_j_rule(self.N) - self.h_j_rule(self.N - 1))
        ...
        DE = DE.at[self.N].set(self.F[self.N] * self.h_feed_rule(self.N) + self.Q_reboiler_rule() \
                     - self.B * (self.h_j_rule(self.N - 1) - self.h_j_rule(self.N)) \
                     - self.F[self.N - 1] * self.h_j_rule(self.N - 1))
        A = diags(
        ...
        self.V = self.V.at[1:].set(linalg.spsolve(A, DE[1:]))
        # self.L[0] = self.RR * self.D
        self.L = self.L.at[0].set(self.RR * self.D)
        for i in range(1, self.N):
            self.L = self.L.at[i].set(self.V[i + 1] - self.D + sum(self.F[k] for k in range(i + 1)))
        self.L = self.L.at[self.N].set(self.B)
...
def make_ABC(V: jnp.array, L: jnp.array, K: jnp.array, F: jnp.array, z: jnp.array,
    ...
    :return: A, B, C, D
    """
    B = jnp.zeros(N + 1)  # diagonal
    A = -1 * jnp.ones(N)  # lower diagonal
    ...
    B = B.at[0].set(1. + Distillate / L[0])
    #C[0] = -V[1] * K[1] / L[1]
    C = C.at[0].set(-V[1] * K[1] / L[1])
    #D[0] = F[0] * z[0]
    D = D.at[0].set(F[0] * z[0])

    # reboiler
    #B[N] = 1 + V[N] * K[N] / Bottoms
    B = B.at[N].set(1 + V[N] * K[N] / Bottoms)
    #D[N] = F[N] * z[N]
    D = D.at[N].set(F[N] * z[N])

    #D[1:N] = F[1:N] * z[1:N]
    D = D.at[1:N].set(F[1:N] * z[1:N])
    #B[1:N] = 1 + V[1:N] * K[1:N] / L[1:N]
    B = B.at[1:N].set(1 + V[1:N] * K[1:N] / L[1:N])
    #C[1:N] = -V[2:(N + 1)] * K[2:(N + 1)] / L[2:(N + 1)]
    C = C.at[1:N].set(-V[2:(N + 1)] * K[2:(N + 1)] / L[2:(N + 1)])
    return A, B, C, D
